...
let arr = [1,2,4]
arr.join('')//将数组转换为字符串'124'
arr.toString()//将数组转换为字符串'1,2,4'
```
伪数组
```js
...
arr.push(9) //在数组最后加个9
//arr = [35,58,862,895,9]
arr.push(258,'amx')//加两个
arr.unshift(1)//在数组头部加个1
//arr = [ 1, 35, 58, 862, 895, 9, 258, "amx"]
arr.splice(2,0,777)//在下标2处，删除0个元素，插入元素777
//arr = [1, 35, 777, 58, 862, 895, 9, 258, "amx"]
```
修改数组元素
```js
arr[2] = 70 //令下标2的元素为70
...
```js
arr.reverse()
//arr = ["amx", 258, 9, 895, 862, 58, 777, 35, 1]
```
...
s.split().reverse().join('') //一次转完
```

数组冒泡
```js
let arr = [35,998,62,15]
